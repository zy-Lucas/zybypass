...
extern "C" void JNIEXPORT Java_net_zy_terminus_Terminus_putMethod(JNIEnv *env, jclass, jstring klass_name,
                                                                  jstring method_name, jstring method_sign,
                                                                  jstring new_klass_name)
{
    // JavaTools::set_method_to_native(klass_name, method_name, method_sign, (uint64_t)ex, env);
    const auto &[method, index]{JavaTools::lookup_method(JavaTools::to_std_string(klass_name),
                                                         JavaTools::to_std_string(method_name),
                                                         JavaTools::to_std_string(method_sign))};
    if (index < 0)
        return;
    hotspot::oops::InstanceKlass ik{method.constants().pool_holder()};
    hotspot::oops::Method new_method{JavaTools::lookup_method(JavaTools::to_std_string(new_klass_name),
                                                              JavaTools::to_std_string(method_name),
                                                              JavaTools::to_std_string(method_sign))
                                         .first};

    JavaTools::replace_method(method.constants().pool_holder(), method, index, new_method);
}
