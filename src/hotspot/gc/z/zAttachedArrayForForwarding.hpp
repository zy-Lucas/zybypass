...
{
  public:
    ZAttachedArrayForForwarding(uint64_t addr) : runtime::JvmObject(addr) {}

    uint64_t length() const noexcept { return read_field<uint64_t>(length_offset); }

    uint64_t object_size() const noexcept;

    ZForwardingEntry get(ZForwarding obj) const noexcept;
